...
**[Live demo →](https://live-suggestions-copilot.vercel.app)** · Paste your Groq API key once, press the mic, start talking. Everything runs session-only — nothing is stored.
...
6. **Role awareness.** Use the `MEETING CONTEXT` field to calibrate: a questioner (seller, interviewer) shouldn't need `answer` suggestions;
...
7. **Mix quality levels.** At least one suggestion per batch should reach beyond the obvious - an intelligent reframe or non-obvious angle.
...
| `suggestionContextChars` | 4000 (~2 min of speech) | 8000 was the original default. Dropping to 4000 reduced token cost |
...
**Chunking strategy.** MediaRecorder with stop/restart every 30 seconds — not the `timeslice` parameter. Timeslice produces fragmented WebM chunks that aren't independently decodable; Whisper rejects them. Stop/restart yields clean, standalone containers per chunk that Whisper can transcribe directly.
...
| Transcript chunk → appears in UI after 30s chunk ends | ~1s | <1ms |
